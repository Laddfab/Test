...
/* NOTA: BLOOM INICIAL */
var INITIAL_BLOOM_STRENGTH = 0.08;
...
var INITIAL_PANEL_OPACITY = 0.00;
...
try {
  ...
  var params = {
    ...
    // Panel (desktop vidrio)
    panelColor: "#ffffff",
    panelOpacity: INITIAL_PANEL_OPACITY,

    // Panel (móvil sólido)
    panelBgMobile: INITIAL_PANEL_BG_MOBILE,
  ...
  // === Panel vidrio (desktop): color + opacidad ===
  ...
  function setPanelOpacity(a){
    ...
    applyPanelBackgroundForResponsive(); // asegura coherencia en móvil/desktop
  }
  setPanelColor(params.panelColor);
  setPanelOpacity(params.panelOpacity);

  gui.addColor(params, "panelColor").name("Panel · Color").onChange(function(v){
    setPanelColor(v);
    applyPanelBackgroundForResponsive();
  });
  gui.add(params, "panelOpacity", 0, 1, 0.01).name("Panel · Opacidad").onChange(setPanelOpacity);

  // === Panel móvil (sólido): color ===
  function applyPanelBackgroundForResponsive(){
    if (!panelEl) return;
    if (isMobile()) {
      // En móvil: color sólido del parámetro
      panelEl.style.background = params.panelBgMobile;
      panelEl.classList.remove("panel-transparent");
    } else {
      // En desktop: deja que gobiernen las CSS variables (vidrio/opacity)
      panelEl.style.background = "";
      // La clase panel-transparent se controla con panelOpacity (arriba)
    }
  }
  gui.addColor(params, "panelBgMobile").name("Panel móvil · Color").onChange(applyPanelBackgroundForResponsive);

  // Inicial y en resize
  applyPanelBackgroundForResponsive();
  window.addEventListener("resize", applyPanelBackgroundForResponsive);
